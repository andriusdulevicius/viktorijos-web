...
const Carousel = ({ projectsData }: { projectsData: ProjectsDataInt[] }) => {
  ...
  useEffect(() => {
    if (isInView) {
      ...
      if (autoRotate) {
        var animationName =
          rotateSpeed > 0 ? `${classes.spin}` : `${classes.spinRevert}`;
    ...
    >
      ...
      <>
        <div className="lg:absolute flex flex-col items-center scale-75 lg:scale-100 justify-center lg:flex-none w-full lg:w-auto  bottom-2 lg:bottom-auto left-4 transition-all ease-in-out duration-300 ">
          ...
          <p className=" text-lg top-52 w-[450px] text-center lg:text-right tracking-[10px] text-[#F5C8D1]/80">
            {duration}
          </p>
          ...
          {title !== 'Hover/click a project to see details' ? (
            <button
              ...
              className="border text-[#6B7280] border-[#F5C8D1] mt-2 py-2 px-10 rounded-md font-bold transition duration-200  ease-in-out hover:bg-[#F5C8D1] hover:text-black lg:w-full w-[300px]"
            >
              Check it out !
            </button>
          ) : null}
