...
import { ProjectsDataInt } from '@/pages';

const ProjectsMobile = ({
  projectsData,
}: {
  projectsData: ProjectsDataInt[];
}) => {
  return (
    <div className="h-screen relative overflow-hidden flex flex-col text-left md:flex-row  justify-center mx-auto items-center z-0 max-w-full ">
      ...
      <motion.div
        ...
        className="relative w-full flex items-center justify-center overflow-y-scroll overflow-y-hidden snap-x snap-mandatory z-20 scrollbar-thin scrollbar-thumb-[#F5C8D1]/80 scrollbar-track-gray-500  scrollbar-thumb-rounded-full scrollbar-track-rounded-full "
      >
        <Swiper
          effect={'cards'}
          grabCursor={true}
        ...
        >
          {projectsData.map((item: any, key: any) => {
            return (
              <SwiperSlide key={key}>
                <div className="flex flex-col  rounded-md h-full w-full bg-[#292929]">
                  <a href={item.projectLink} target="_blank" rel="noreferrer">
                    ...
                    <p className="text-center mt-2 font-medium tracking-widest text-[20px] text-[#F5C8D1]">
                      {item.projectName}
                    </p>
                    <p className="text-center mt-1 font-thin tracking-wider text-[15px] text-[#F5C8D1]">
